...
beforeEach(() => {
  ...
  vscodeTest.runTests.mockImplementation(async (options) => {
    await writeFile(
      options.extensionTestsEnv.ECHADRON_VSCODE_SMOKE_REPORT,
...
describe('installed VSIX Extension Host smoke', () => {
  ...
  it('gives the harness separate Echadron and operating-system homes before activation', async () => {
    ...
    expect(env.ECHADRON_HOME).not.toBe(env.ECHADRON_VSCODE_SMOKE_OS_HOME);
    expect(env.ECHADRON_VSCODE_SMOKE_OS_HOME).toContain('os-home');
  });
