...
A browser client for Echadron — a peer to the terminal UI that talks to a local
...
ECHADRON_WEB_PORT=5197 ECHADRON_SERVER_URL=http://192.168.97.91:58627 pnpm -C apps/echadron-web run dev
#   …or from the repo root:  pnpm dev:web   (uses the defaults below)

# checks
pnpm -C apps/echadron-web run typecheck     # vue-tsc --noEmit
pnpm -C apps/echadron-web run test          # vitest (pure logic only)
pnpm -C apps/echadron-web run build         # vite build
```
...
| --------------------- | ----------------------- | ----------------------------------------------- |
| `ECHADRON_WEB_PORT`   | `5175`                  | port the dev server listens on                 |
| `ECHADRON_SERVER_URL` | `http://127.0.0.1:58627` | where `/api/v1` (and `/api/v1/ws`) is forwarded |

The legacy `WEB_PORT` and `KIMI_SERVER_URL` variables remain accepted as
fallback aliases. `ECHADRON_BACKEND_DEFAULT_URL` and
`ECHADRON_BACKEND_MULTI_URL` likewise supersede the legacy
`KIMI_BACKEND_DEFAULT_URL` and `KIMI_BACKEND_MULTI_URL` names.
...
server (REST + WS)
  ...
  └─ src/composables/useEchadronWebClient.ts   the ONLY place that imports api + state;
...
Echadron Web is **not published as a standalone package**. It ships as the
built-in web UI of the Echadron CLI.

### Current release flow

1. **Develop** — `pnpm dev:web` (or `pnpm -C apps/echadron-web run dev`).
2. **Build** — `pnpm -C apps/echadron-web run build` produces `apps/echadron-web/dist`.
3. **Bundle into the CLI** — the CLI build runs `scripts/copy-web-assets.mjs`,
   which copies `apps/echadron-web/dist` into the CLI's web asset directory.
4. **Publish** — the release workflow publishes the CLI package; the web assets
   travel with that package.
5. **Serve** — `echadron web` serves the bundled web assets.

The web UI does not display its own package version or build commit. It is
bundled into the CLI package and follows the CLI release.

### Suggested improvements

- **Keep the current coupling for now.** Because Echadron is primarily a local
...
- **Add an independent web-deploy workflow only when needed.** If a public
  ...
  `.github/workflows/web-deploy.yml` that builds `apps/echadron-web` and uploads
...
- **Keep versioning owned by the CLI release.** `apps/echadron-web/package.json`
...
- **Ensure the web build is exercised in CI.** The root `build` script already
  builds every workspace, so `pnpm run build` in CI covers `apps/echadron-web`.
