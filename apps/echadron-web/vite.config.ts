...
const webPort = Number(process.env.ECHADRON_WEB_PORT || process.env.WEB_PORT) || 5175;
...
// both can run at once) for multi-instance debugging. Override with the
// ECHADRON_BACKEND_* variables.
...
// preset; ECHADRON_SERVER_URL pins the initial target (and disables nothing —
// the dev switcher can still move it at runtime).
...
function backendSwitcherPlugin(): Plugin {
  ...
  return {
    name: 'echadron-backend-switcher',
    configureServer(server) {
      server.middlewares.use((req: IncomingMessage, res: ServerResponse, next: () => void) => {
        if (req.url !== '/__echadron-dev/backend') return next();
        if (req.method === 'GET') {
          sendJson(res, state());
          return;
        }
...
export default defineConfig({
  plugins: [
    ...
    Icons({
      compiler: 'vue3',
      // Local Echadron Design System icons (24×24 outlined, fill="currentColor"),
      // kept in the historical `src/icons/echadron/` collection so existing icon
      // imports remain stable. Registered in src/lib/icons.ts only.
      customCollections: {
        echadron: FileSystemIconLoader(fileURLToPath(new URL('./src/icons/echadron', import.meta.url))),
      },
    }),
  ],
  ...
  define: {
    ...
    __ECHADRON_WEB_DESKTOP__: JSON.stringify(process.env.ECHADRON_WEB_DESKTOP === '1'),
  },
  server: {
    port: webPort,
    strictPort: false,
  ...
  preview: {
    port: Number(process.env.ECHADRON_WEB_PREVIEW_PORT || process.env.WEB_PREVIEW_PORT) || 4175,
    proxy: {
      '/api/v1': apiProxyOptions,
    },
  },
