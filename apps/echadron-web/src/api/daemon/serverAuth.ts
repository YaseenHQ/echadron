// apps/echadron-web/src/api/daemon/serverAuth.ts
...
// and WebSocket call (the persistent server token, or the ECHADRON_CODE_PASSWORD
// password). The Web UI obtains that credential in one of two ways:
//   1. From the URL fragment (`#token=<...>`) that `echadron web` appends when it
...
// at <ECHADRON_CODE_HOME>/server.token and handed to the browser in the launch URL.
// `echadron web rotate-token` invalidates a stale copy, and the next 401 clears
// it here.

const STORAGE_KEY = 'echadron-web.server-credential';
const LEGACY_STORAGE_KEY = 'kimi-web.server-credential';
...
function readLocalCredentialRaw(): string | null {
  const current = globalThis.localStorage?.getItem(STORAGE_KEY) ?? null;
  if (current !== null) return current;
  const legacy = globalThis.localStorage?.getItem(LEGACY_STORAGE_KEY) ?? null;
  if (legacy !== null) {
...
function removeLocalCredentialRaw(): void {
  globalThis.localStorage?.removeItem(STORAGE_KEY);
  globalThis.localStorage?.removeItem(LEGACY_STORAGE_KEY);
}

function removeSessionCredentialRaw(): void {
...
function loadStored(): StoredCredential | undefined {
  try {
    const raw = readLocalCredentialRaw();
    if (raw) {
      ...
      if (stored === undefined) {
        ...
        if (!migrationRecorded) {
          try {
            if (readLocalCredentialRaw() === raw) {
              removeLocalCredentialRaw();
            }
            migrationRecorded = true;
          } catch {
    ...
    const legacy = globalThis.sessionStorage?.getItem(STORAGE_KEY) ??
      globalThis.sessionStorage?.getItem(LEGACY_STORAGE_KEY);
    if (legacy) {
...
export function clearCredential(): void {
  ...
  try {
    ...
    // (e.g. right after `echadron web rotate-token`).
    const raw = readLocalCredentialRaw();
    const stored = raw === null || raw === undefined
      ? undefined
      : decodeStoredCredential(raw);
