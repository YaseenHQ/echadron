// apps/echadron-web/src/api/daemon/eventReducer.ts
// Pure TypeScript state reducer for EchadronClient.
...
const OPTIMISTIC_USER_MESSAGE_METADATA_KEY = 'echadronWeb.optimisticUserMessage';
const LEGACY_OPTIMISTIC_USER_MESSAGE_METADATA_KEY = 'kimiWeb.optimisticUserMessage';
...
export interface EchadronClientState {
  sessions: AppSession[];
...
export function createInitialState(): EchadronClientState {
  return {
    sessions: [],
...
function cloneState(s: EchadronClientState): EchadronClientState {
  return {
    ...s,
...
function advanceSeq(state: EchadronClientState, sessionId: string | undefined, seq: number | undefined): void {
...
function isOptimisticUserMessage(message: AppMessage): boolean {
  return (
    ...
    (message.metadata?.[OPTIMISTIC_USER_MESSAGE_METADATA_KEY] === true ||
      message.metadata?.[LEGACY_OPTIMISTIC_USER_MESSAGE_METADATA_KEY] === true)
  );
}
...
export function reduceAppEvent(
  ...
  switch (event.type) {
    ...
    // (see useEchadronWebClient.processEvent), not reducer state. Advance seq
    // silently.
