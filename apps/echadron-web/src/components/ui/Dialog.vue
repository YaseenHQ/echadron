<!-- apps/echadron-web/src/components/ui/Dialog.vue -->
...
.ui-dialog__overlay {
  ...
  animation: echadron-dialog-overlay-in var(--duration-base) var(--ease-out);
}
@keyframes echadron-dialog-overlay-in {
  from { opacity: 0; }
  to { opacity: 1; }
}
.ui-dialog {
  ...
  outline: none;
  overflow: hidden;
  animation: echadron-card-in var(--duration-slow) var(--ease-out);
}
