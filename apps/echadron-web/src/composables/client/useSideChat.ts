// apps/echadron-web/src/composables/client/useSideChat.ts
...
import type { AppMessage, EchadronEventConnection, ThinkingLevel } from '../../api/types';
...
export function useSideChat(rawState: ExtendedState, deps: UseSideChatDeps) {
  ...
  async function openSideChatOn(parent: string, initialPrompt?: string): Promise<void> {
    if (!sideChatTargetBySession.value[parent]) {
      let agentId: string;
      try {
        ({ agentId } = await getEchadronWebApi().startBtw(parent));
      } catch (err) {
  ...
  async function sendSideChatPromptOn(parent: string, text: string): Promise<void> {
    ...
    try {
      ...
      const result = await getEchadronWebApi().submitPrompt(sid, {
        content: [{ type: 'text', text: trimmed }],
        agentId,
        model,
