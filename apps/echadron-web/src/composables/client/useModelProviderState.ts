// apps/echadron-web/src/composables/client/useModelProviderState.ts
...
import type { ExtendedState } from '../useEchadronWebClient';
...
export function useModelProviderState(
  ...
  async function setModel(modelId: string): Promise<boolean> {
    ...
    try {
      await getEchadronWebApi().updateSession(sid, {
        model: modelId,
  ...
  async function activateSkill(skillName: string, args?: string, sessionId?: string): Promise<void> {
    ...
    if (guarded) {
      ...
      const optimisticMsg: AppMessage = {
        ...
        metadata: {
          'echadronWeb.optimisticUserMessage': true,
          origin: {
    ...
    try {
      ...
      await getEchadronWebApi().activateSkill(sid, skillName, args);
    } catch (err) {
      if (guarded) {
  ...
  async function refreshProvider(id: string): Promise<void> {
    try {
      const result = await getEchadronWebApi().refreshProvider(id);
  ...
  async function refreshAllProviders(): Promise<void> {
    try {
      const result = await getEchadronWebApi().refreshAllProviders();
      for (const failure of result.failed) {
  ...
  /** Start the managed provider OAuth device flow. Returns flow data or null on error. */
  ...
  async function pollOAuthLogin(): Promise<{
    ...
    try {
      ...
      console.warn('[echadron-web] pollOAuthLogin failed', err);
      return null;
    }
  }
