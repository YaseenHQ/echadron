// apps/echadron-web/src/composables/client/useWorkspaceState.ts
...
import { getEchadronWebApi } from '../../api';
...
import type { ExtendedState, PromptAttachment } from '../useEchadronWebClient';
...
export interface UseWorkspaceStateDeps {
  ...
  getEventConn: () => EchadronEventConnection | null;
...
export function useWorkspaceState(rawState: ExtendedState, deps: UseWorkspaceStateDeps) {
  ...
  async function loadOlderMessages(sessionId: string): Promise<void> {
    ...
    try {
      const page = await getEchadronWebApi().listMessages(sessionId, {
        beforeId,
  ...
  async function loadFileDiff(path: string): Promise<void> {
    ...
    try {
      ...
      // empty. Surfacing it as a global server API error toast on a routine
  ...
  async function loadMoreSessions(workspaceId: string): Promise<void> {
    ...
    try {
      const page = await getEchadronWebApi().listSessions({
        workspaceId,
  ...
  async function refreshServerMeta(): Promise<void> {
    const m = await getEchadronWebApi()
      .getMeta()
      .catch(() => null);
    if (m === null) return;
  ...
  async function fetchSessionIntoList(sessionId: string): Promise<boolean> {
    try {
      const session = await getEchadronWebApi().getSession(sessionId);
  ...
  async function submitPromptInternal(
    ...
    try {
      ...
      const optimisticMsg: AppMessage = {
        ...
        metadata: { 'echadronWeb.optimisticUserMessage': true },
      };
  ...
  async function createGoal(objective: string): Promise<void> {
    ...
    try {
      await getEchadronWebApi().updateSession(sid, { goalObjective: trimmed });
    } catch (err) {
  ...
  function controlGoal(action: 'pause' | 'resume' | 'cancel'): void {
    const sid = rawState.activeSessionId;
    if (!sid) return;
    void Promise.resolve(getEchadronWebApi().updateSession(sid, { goalControl: action }))
      .catch((err) => {
  ...
  async function renameWorkspace(id: string, name: string): Promise<void> {
    ...
    try {
      await getEchadronWebApi().updateWorkspace(id, { name });
  ...
  async function deleteWorkspace(id: string): Promise<void> {
    ...
    try {
      await getEchadronWebApi().deleteWorkspace(id);
    } catch (err) {
  ...
  async function exportSession(targetSessionId?: string): Promise<void> {
    ...
    try {
      ...
      const { blob, fileName } = await getEchadronWebApi().exportSession(sessionId, webLog);
  ...
  async function restoreSession(id: string): Promise<boolean> {
    try {
      const restored = await getEchadronWebApi().restoreSession(id);
  ...
  function loadArchivedSessions(input?: { beforeId?: string; pageSize?: number }) {
    return getEchadronWebApi().listSessions({
      archivedOnly: true,
  ...
  /** Logout from the managed provider (OAuth). Re-checks auth and reloads sessions. */
  ...
  async function forkSession(sessionId?: string): Promise<void> {
    ...
    if (!sid) return;
    try {
      const forked = await getEchadronWebApi().forkSession(sid);
  ...
  async function undo(count = 1): Promise<string | null> {
    ...
    try {
      await getEchadronWebApi().undoSession(sid, count);
  ...
  function getFileDownloadUrl(path: string): string | null {
    const sid = rawState.activeSessionId;
    if (!sid) return null;
    return getEchadronWebApi().getFileDownloadUrl(sid, path);
  }
  ...
  async function openWorkspaceFile(path: string, line?: number): Promise<boolean> {
    ...
    if (!sid) return false;
    try {
      await getEchadronWebApi().openFile(sid, { path, line });
      return true;
    } catch (err) {
  ...
  async function openInApp(appId: string): Promise<void> {
    ...
    try {
      await getEchadronWebApi().openInApp(sid, appId, path);
    } catch (err) {
  ...
  async function revealWorkspaceFile(path: string): Promise<boolean> {
    ...
    if (!sid) return false;
    try {
      await getEchadronWebApi().revealFile(sid, { path });
      return true;
    } catch (err) {
