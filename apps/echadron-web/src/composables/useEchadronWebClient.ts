// apps/echadron-web/src/composables/useEchadronWebClient.ts
...
import { createInitialState, reduceAppEvent, type CompactionStatus, type EchadronClientState } from '../api/daemon/eventReducer';
...
// existing `import type { ColorScheme, Accent } from './useEchadronWebClient'`
...
// The UI theme (terminal / modern variants) was retired in favor of a single
...
export interface ExtendedState extends EchadronClientState {
  connected: boolean;
...
async function refreshSessionStatus(sessionId: string): Promise<void> {
  ...
  try {
    st = await getEchadronWebApi().getSessionStatus(sessionId);
  } catch {
...
async function refreshSessionGoal(sessionId: string): Promise<void> {
  ...
  try {
    goal = await getEchadronWebApi().getSessionGoal(sessionId);
  } catch {
...
function persistSessionProfile(patch: {
  ...
  return Promise.resolve(getEchadronWebApi().updateSession(sid, patch))
...
let eventConn: EchadronEventConnection | null = null;
...
function applyEvent(event: ReturnType<typeof toAppEvent>, sessionId: string, seq: number): void {
  const snapshot: EchadronClientState = {
...
function processEvent(appEvent: AppEvent, meta: EchadronEventMeta): void {
...
async function pullSessionWarnings(sessionId: string): Promise<void> {
  ...
  try {
    const warnings = await getEchadronWebApi().getSessionWarnings(sessionId);
...
const turns = computed<ChatTurn[]>(() => {
  ...
  return messagesToTurns(
    messages,
    approvals,
    (fileId) => getEchadronWebApi().getFileUrl(fileId),
    turnActive.value,
    rawState.planReviewByToolCallId,
  );
});
...
export function useEchadronWebClient() {
