// apps/echadron-web/src/composables/useFilePreview.ts
...
import type { useEchadronWebClient } from './useEchadronWebClient';

type EchadronWebClient = ReturnType<typeof useEchadronWebClient>;
...
export function useFilePreview({ client, detailTarget }: UseFilePreviewOptions) {
  ...
  function openMediaPreview(media: ToolMedia): void {
    ...
    if (media.fileId) {
      ...
      void getEchadronWebApi().getFileBlob(media.fileId).then((blob) => {
