...
import type { AppTerminal, EchadronEventConnection } from '../api/types';
...
export function useTerminal(sessionId: Ref<string>) {
  ...
  let conn: EchadronEventConnection | null = null;

  function ensureConnection(): EchadronEventConnection | null {
    if (conn !== null) return conn;
    if (typeof WebSocket === 'undefined') return null;
    conn = getEchadronWebApi().connectEvents({
      onEvent: () => {},
      onResync: () => {},
      onError: (_code, msg) => {
        error.value = msg;
  ...
  async function close(): Promise<void> {
    ...
    try {
      ...
      await getEchadronWebApi().closeTerminal(current.sessionId, current.id);
    } catch (error_) {
      error.value = error_ instanceof Error ? error_.message : String(error_);
    }
  }
