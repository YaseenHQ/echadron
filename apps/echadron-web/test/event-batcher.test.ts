 ...
 * Run: pnpm --filter @yaseenhq/echadron-web exec vitest run test/event-batcher.test.ts
 */

import { describe, expect, it, vi } from 'vitest';

import { createInitialState, reduceAppEvent, type EchadronClientState } from '../src/api/daemon/eventReducer';
import type {
  AppEvent,
  AppMessage,
  AppSession,
...
vi.mock('../src/api', () => ({
  getEchadronWebApi: () => clientApiMock,
}));
...
function assistantState(content: AppMessage['content'] = [{ type: 'text', text: '' }]): EchadronClientState {
...
describe('useEchadronWebClient (resync integration)', () => {
  it('flushes queued deltas around an authoritative snapshot before live streaming resumes', async () => {
    ...
    let handlers: EchadronEventHandlers | undefined;
    ...
    let seedCalls = 0;
    const connection: EchadronEventConnection = {
      subscribe: vi.fn(),
      unsubscribe: vi.fn(),
    ...
    const api: Partial<EchadronWebApi> = {
      getAuth: vi.fn(async () => ({
        ready: true,
    ...
    try {
      const { useEchadronWebClient } = await import('../src/composables/useEchadronWebClient');
      const client = useEchadronWebClient();
      await client.load();
