...
describe('draftStorageKey', () => {
  it('uses the session id when present', () => {
    expect(draftStorageKey('abc')).toBe('echadron-web.draft.abc');
  });

  it('falls back to __new__ when sid is empty/undefined', () => {
    expect(draftStorageKey(undefined)).toBe('echadron-web.draft.__new__');
    expect(draftStorageKey('')).toBe('echadron-web.draft.__new__');
  });
});

describe('STORAGE_KEYS', () => {
  it('keeps the legacy key strings unchanged', () => {
    expect(STORAGE_KEYS.theme).toBe('echadron-web.theme');
    expect(STORAGE_KEYS.activeWorkspace).toBe('echadron-active-workspace');
    expect(STORAGE_KEYS.notifyOnComplete).toBe('echadron-web.notify-on-complete');
    expect(STORAGE_KEYS.notifyOnQuestion).toBe('echadron-web.notify-on-question');
    expect(STORAGE_KEYS.soundOnComplete).toBe('echadron-web.sound-on-complete');
    expect(STORAGE_KEYS.locale).toBe('echadron-locale');
  });
});
