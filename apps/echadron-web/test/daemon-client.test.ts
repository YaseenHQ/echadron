// apps/echadron-web/test/daemon-client.test.ts
// DaemonEchadronWebApi public REST adapter: session export binary/error contracts,
...
// Run: pnpm --filter @yaseenhq/echadron-web exec vitest run test/daemon-client.test.ts

import { afterEach, beforeEach, describe, expect, it, vi } from 'vitest';

import { DaemonEchadronWebApi } from '../src/api/daemon/client';
import { DaemonApiError, DaemonNetworkError } from '../src/api/errors';
import { clearTrace, traceToJsonl } from '../src/debug/trace';
import type { AppEvent, EchadronEventConnection, EchadronEventMeta } from '../src/api/types';
...
function createApi(): DaemonEchadronWebApi {
  return new DaemonEchadronWebApi({
...
describe('DaemonEchadronWebApi.exportSession', () => {
  beforeEach(() => {
...
describe('DaemonEchadronWebApi.getSessionGoal', () => {
  beforeEach(() => {
    vi.stubGlobal('fetch', vi.fn());
  });
...
describe('DaemonEchadronWebApi.connectEvents', () => {
  let connection: EchadronEventConnection | undefined;
  ...
  it('delivers raw assistant stream coordinates with the projected delta', () => {
    ...
    const received: Array<{ event: AppEvent; meta: EchadronEventMeta }> = [];
