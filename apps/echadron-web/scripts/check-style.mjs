#!/usr/bin/env node
// check-style.mjs — design-system §06 anti-pattern guard for apps/echadron-web.
//
...
// illustrations, the spinner, and brand marks (the Echadron wordmark on the loading
// screen). Everything else should use lib/icons.ts via <Icon>/iconSvg(). The
// 32x22 Echadron mark is also exempted inline (matched by viewBox). The icon
...
function checkFile(abs) {
  ...
  if (!isCss && !ICON_EXEMPT.has(file)) {
    ...
    while ((m = svgRe.exec(content)) !== null) {
      ...
      if (/viewBox="0 0 32 22"/.test(m[0])) continue; // Echadron brand mark
