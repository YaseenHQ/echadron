...
| --- | --- | --- |
| `@yaseenhq/echadron` | `apps/kimi-code` | Echadron CLI / TUI application — provides the `echadron`, `chad`, and `maker` commands |
...
| --- | --- |
| Only modifies TUI behavior in `@yaseenhq/echadron` | Add `patch` / `minor` / `major` to `@yaseenhq/echadron` |
| Only modifies internal packages, no user-visible change in SDK / CLI | Usually no changeset needed |
| Internal package fix changes the CLI user experience | Add a changeset to `@yaseenhq/echadron` describing the user-visible fix |
| Internal package adds a new capability exposed by the SDK | Add a changeset to `@moonshot-ai/kimi-code-sdk` |
| SDK behavior change affects CLI user experience | Add changesets to both `@moonshot-ai/kimi-code-sdk` and `@yaseenhq/echadron` |
| Provider abstraction change affects SDK / CLI | Add changesets to the affected `@moonshot-ai/kimi-code-sdk` and/or `@yaseenhq/echadron` |
...
1. Open each publishable package's page on the npm website, e.g. `https://www.npmjs.com/package/@yaseenhq/echadron`.
...
- If a change in an underlying internal package alters user-visible behavior or public API of a publishable package, add a changeset to the affected publishable package. For example, when a bug fixed in `@moonshot-ai/agent-core` resolves an issue CLI users encounter, add a changeset to `@yaseenhq/echadron` describing the user-visible fix.
- `@yaseenhq/echadron` is the official Echadron CLI package name; after a global install it provides the `echadron`, `chad`, and `maker` commands.
