...
import EchadronLogo from './EchadronLogo.vue'
...
const copy = computed(() => isZh.value
  ? {
      ...
      secondaryHref: 'https://github.com/YaseenHQ/kimi',
    }
  : {
      titleLead: 'Echadron',
      titleAccent: 'Code',
...
<template>
  <section class="EchadronHero">
    <div class="EchadronHero__halo" aria-hidden="true" />
    <div class="EchadronHero__inner">
      <div class="EchadronHero__logo">
        <EchadronLogo :size="64" />
      </div>
      <h1 class="EchadronHero__title">
        {{ copy.titleLead }}&nbsp;<span class="EchadronHero__accent">{{ copy.titleAccent }}</span>{{ copy.titleTail }}
      </h1>
      <p class="EchadronHero__tagline">{{ copy.tagline }}</p>
      <div class="EchadronHero__actions">
        <a class="EchadronBtn EchadronBtn--primary" :href="withBase(copy.primaryHref)">
          {{ copy.primaryText }}
          <svg width="16" height="16" viewBox="0 0 16 16" fill="none" aria-hidden="true">
            <path d="M6 3l5 5-5 5" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
          </svg>
        </a>
        <a class="EchadronBtn EchadronBtn--ghost" :href="copy.secondaryHref" target="_blank" rel="noopener">
...
:global(.dark) .EchadronHero__halo {
  opacity: 0.85;
  background:
...
.EchadronHero__accent {
  background: var(--echadron-brand-gradient);
...
@media (max-width: 480px) {
  ...
  .EchadronHero__actions .EchadronBtn {
    width: 100%;
  }
}
</style>
