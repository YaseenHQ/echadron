...
Echadron 有三个地方可以影响运行参数：配置文件、命令行选项、环境变量。它们不是简单的"谁优先级高谁赢"——三者面向不同场景，作用范围互不相同：
...
1. **定位配置文件**：`ECHADRON_HOME` 决定数据根目录，配置文件路径因此变为 `$ECHADRON_HOME/config.toml`。这一步先于其他所有解析，不是普通参数的后备来源。
...
2. **用户配置文件**（`~/.echadron/config.toml`）：保存长期偏好
...
目前 CLI 只读取一份用户级配置文件，没有项目级配置文件机制。需要在不同项目间隔离配置时，用 `ECHADRON_HOME` 指向不同的数据目录——见下文[典型场景](#典型场景)。
...
```sh
ECHADRON_HOME="$PWD/.kimi-sandbox" kimi
```
...
- [环境变量](./env-vars.md) — `ECHADRON_HOME` 等变量的完整列表与说明
