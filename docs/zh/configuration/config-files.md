...
Echadron 把所有长期偏好写进 `~/.echadron/` 下的 TOML（一种结构清晰的纯文本配置格式）文件——比如使用哪个模型、填哪个 API 密钥、Agent 每轮最多跑几步。改一次，每次启动都生效。Agent 与运行时设置放在 `config.toml`，终端界面与客户端偏好（主题、编辑器、通知、自动更新）放在配套的 `tui.toml`。

默认位置：`~/.echadron/config.toml`，首次运行时自动创建。

## 配置文件位置

CLI 从 `~/.echadron/config.toml` 读取配置。如需把数据目录迁移到别处，可用 `ECHADRON_HOME` 环境变量覆盖：

```sh
export ECHADRON_HOME=/path/to/kimi-home
```

此时配置文件路径变为 `$ECHADRON_HOME/config.toml`。无论目录在哪里，文件名固定是 `config.toml`。
...
| 字段 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| `type` | `string` | 是 | 供应商类型：`echadron`、`anthropic`、`openai`、`openai_responses`、`google-genai`、`vertexai` |
...
该功能默认开启，并在包括交互式 TUI 在内的所有启动方式下生效。为便于回退，仍保留 `[experimental]` 控制项：设置 `secondary-model = false`，或使用 `ECHADRON_EXPERIMENTAL_SECONDARY_MODEL=0`。历史名称 `KIMI_CODE_EXPERIMENTAL_SECONDARY_MODEL` 仍可作为别名使用。
...
Echadron 已完成的功能默认开启。历史 `[experimental]` 表继续作为兼容配置命名空间，用于明确回退；同样的开关也可在**设置 → 功能控制**中调整：
...
三个开关分别关闭渐进式 MCP 工具披露、次主力模型路由和 minidb 派生会话读取模型。ACP v1/v2 与 MCP 协议协商属于正常传输行为，因此不是功能开关。
...
| `max_attempts_per_step` | `integer` | `10` | 单步失败后的最大尝试次数 |
| `reserved_context_size` | `integer` | — | 预留给模型输出的 token 数；上下文窗口剩余量低于此值时触发自动压缩 |

`max_steps_per_turn` 可被环境变量 `KIMI_LOOP_MAX_STEPS_PER_TURN` 覆盖，`max_attempts_per_step` 可被 `KIMI_LOOP_MAX_ATTEMPTS_PER_STEP` 覆盖，优先级均高于配置文件。旧字段 `max_retries_per_step` 与旧变量 `KIMI_LOOP_MAX_RETRIES_PER_STEP` 仍作为兼容回退，并会由 `echadron doctor` 提示已弃用。
...
| `print_wait_ceiling_s` | `integer` | `315360000` | print 模式（`echadron -p`）下，`print_background_mode` 为 `"drain"` 或 `"steer"` 时，等待/steer 循环的墙钟上限（秒；默认 10 年，近似不设限）。在非 print 模式或 `"exit"` 时无效 |
| `print_max_turns` | `integer` | `100000` | print 模式（`echadron -p`）且 `print_background_mode = "steer"` 时，允许由后台任务完成触发的新 turn 的最大数量，防止 steer 循环失控（默认值近似不设限） |
...
::: tip
MCP server 的声明配置写在 `~/.echadron/mcp.json` 或项目内 `.echadron/mcp.json` 中，不在 `config.toml` 里。交互式配置入口是 `/mcp-config`，详见 [Model Context Protocol](../customization/mcp.md)。
:::

## `tui.toml`

除了 `config.toml`，CLI 还在同一目录下用一份配套的 `tui.toml` 保存终端界面与客户端偏好（`~/.echadron/tui.toml`，或覆盖后的 `$ECHADRON_HOME/tui.toml`）。它在首次运行时以默认值创建，交互式命令 `/config`、`/theme`、`/editor` 会自动写入，通常无需手动编辑。文件格式有误时，CLI 会回退到默认值并给出提示，而不是启动失败。
...
目录以绝对路径存储，与具体机器相关。因此建议把 `.echadron/local.toml` 加入项目的 `.gitignore`，避免被提交。

## 下一步

- [平台与模型](./providers.md) — 各供应商类型（Kimi、Claude、OpenAI、Gemini）的接入示例
- [配置覆盖](./overrides.md) — CLI 选项、配置文件、环境变量的优先级规则
- [环境变量](./env-vars.md) — `ECHADRON_HOME` 等运行时变量的完整列表
