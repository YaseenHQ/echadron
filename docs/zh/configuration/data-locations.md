...
Echadron 把所有运行时数据——配置文件、会话历史、登录凭据、诊断日志——集中存放在 `~/.echadron/` 下。本页帮你搞清楚每类数据在哪里、用来做什么，以及需要时怎么清理或搬迁。

## 数据根目录

默认数据根是 `~/.echadron/`，在不同平台的实际路径：

- macOS：`/Users/<name>/.echadron`
- Linux：`/home/<name>/.echadron`
- Windows：`C:\Users\<name>\.echadron`

如果你需要把数据目录挪到别处（比如用多个独立环境隔离不同项目的配置），设置 `ECHADRON_HOME` 即可：

```sh
export ECHADRON_HOME="$HOME/.config/echadron"
```

设置后，配置、会话、日志、OAuth 凭据、Echadron 专属用户级 Skills、全局 `AGENTS.md` 等 **Echadron 数据**都会落到新路径下。`ECHADRON_HOME` 的完整说明见[环境变量](./env-vars.md)。旧的 `IMPERIUM_HOME` 和 `KIMI_CODE_HOME` 仍可作为数据根兼容别名；Echadron 不安装 `kimi` 命令。

::: tip 提示

**通用 `.agents` 资源**仍放在真实 OS home 下，以便跨工具共享。例如，用户级通用 Skills 仍位于 `~/.agents/skills/`，而 Echadron 专属用户级 Skills 会随 `ECHADRON_HOME` 移动到 `$ECHADRON_HOME/skills/`。
:::

## 目录结构

```
$ECHADRON_HOME  （默认 ~/.echadron）
├── config.toml             # 用户配置
├── tui.toml                # 终端界面偏好（含自动更新开关）
├── AGENTS.md               # 全局 Echadron 专属 Agent 指令（可选）
├── mcp.json                # 用户级 MCP server 声明（可选）
├── skills/                 # Echadron 专属用户级 Skills（可选）
├── plugins/
...
- **`AGENTS.md`**：全局 Echadron 专属 Agent 指令。该文件会随 `ECHADRON_HOME` 移动；跨工具通用指令仍可放在 `~/.agents/AGENTS.md`。
- **`mcp.json`**：用户级 MCP server 声明，启动时与项目内的 `.echadron/mcp.json` 合并加载。详见 [MCP](../customization/mcp.md)。
- **`skills/`**：Echadron 专属用户级 Skills。该目录会随 `ECHADRON_HOME` 移动；跨工具通用 Skills 仍可放在 `~/.agents/skills/`。详见 [Agent Skills](../customization/skills.md)。
...
- **`cron/`**：定时任务持久化，用 `echadron --session` 恢复会话时重新加载到调度器。详见[定时任务](../reference/tools.md#定时任务)。
...
报 bug 时，优先用 `echadron export` 导出相关会话（详见 [Echadron 命令](../reference/kimi-command.md)）；会话日志默认包含在导出包里。不想分享全局日志时加 `--no-include-global-log`。
...
删除数据根目录（`~/.echadron/` 或 `ECHADRON_HOME` 指定路径）可清除所有运行时数据。只需清理部分内容时：

| 需求 | 操作 |
| --- | --- |
| 重置配置 | 删除 `~/.echadron/config.toml` |
| 重置终端界面偏好 | 删除 `~/.echadron/tui.toml` |
| 清理所有会话 | 删除 `~/.echadron/sessions/` 和 `session_index.jsonl` |
| 清理诊断日志 | 删除 `~/.echadron/logs/` |
| 清理输入历史 | 删除 `~/.echadron/user-history/` |
| 重置更新状态 | 删除 `~/.echadron/updates/latest.json` |
| 强制重新下载托管 `rg` 和 `fd` | 删除 `~/.echadron/bin/` |
| 清除供应商 OAuth 登录态 | 运行 `/logout`，或删除对应的 `credentials/<name>.json` |
| 清除 MCP server OAuth 登录态 | 删除 `credentials/mcp/`（`/logout` 不会清理 MCP 凭据） |
| 移除用户级 MCP 声明 | 删除 `$ECHADRON_HOME/mcp.json`（默认为 `~/.echadron/mcp.json`） |
| 清理全局 Echadron 专属 Agent 指令 | 删除 `$ECHADRON_HOME/AGENTS.md`（默认为 `~/.echadron/AGENTS.md`） |
| 清理 plugin 安装记录 | 删除 `$ECHADRON_HOME/plugins/`（本地 plugin 源码不受影响） |
| 清空 Echadron 专属用户级 Skills | 删除 `$ECHADRON_HOME/skills/`（默认为 `~/.echadron/skills/`） |

## 下一步

- [配置文件](./config-files.md) — `config.toml` 各字段的完整说明
- [环境变量](./env-vars.md) — `ECHADRON_HOME` 等路径变量的详细用法
