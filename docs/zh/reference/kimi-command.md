...
所有 flag 都是可选的，直接运行 `echadron` 即可进入交互式会话：
...
恢复会话时，可以通过 `--auto`、`--yolo` 或 `--plan` 覆盖原会话保存的权限或计划模式。例如，`echadron --continue --auto` 会恢复最近会话并切换到 auto 权限模式。
...
`--agent` 和 `--agent-file` 用于选择驱动新会话的 Agent，在 print 模式（`echadron -p`）和交互式 TUI 中均可使用：

```sh
echadron --agent reviewer
echadron -p --agent reviewer "审查这个分支上的改动"
```
...
`echadron` 提供以下子命令：`login`（非交互式登录）、`acp`（ACP IDE 模式）、`web`（前台运行本地 REST/WebSocket/web 服务并打开 web UI）、`doctor`（校验配置文件）、`export`（导出会话）、`migrate`（迁移旧版数据）、`upgrade`（检查更新）、`provider`（管理供应商）。

### `echadron login`

通过 RFC 8628 device-code 流程登录 Kimi Code OAuth，无需进入 TUI。命令会发起一次 device authorization 请求，将验证地址和用户码打印到 stderr，然后轮询直到浏览器侧完成授权。生成的 token 写入与 TUI `/login` 相同的本地位置，下次启动 `echadron` 时会自动加载。
...
```sh
echadron web                 # 前台运行服务并打开浏览器
echadron web --no-open       # 不打开浏览器
echadron web --port 58628    # 指定绑定端口
```

同一 home 目录下可以同时运行多个实例：每个实例注册到 `~/.echadron/server/instances/`，端口被占用时自动 +1 重试（58628、58629……）。
...
生成新的持久化 bearer token（写入 `~/.echadron/server.token`），旧 token 立即失效。token 是整个 home 目录共享的，所有运行中的实例会在下一次鉴权校验时自动换用新 token，无需重启。

### `echadron doctor`

校验 `config.toml` 和 `tui.toml`，不会启动 TUI，也不会修改任一文件。默认检查 `ECHADRON_HOME` 下的文件；未设置该环境变量时检查 `~/.echadron`。默认路径缺失时会显示为跳过，因为内置默认值仍可生效。
...
| 命令 | 说明 |
| --- | --- |
| `echadron doctor` | 校验默认 `config.toml` 和 `tui.toml` |
| `echadron doctor config [path]` | 只校验 `config.toml`；传入 `path` 时使用该文件而不是默认文件 |
| `echadron doctor tui [path]` | 只校验 `tui.toml`；传入 `path` 时使用该文件而不是默认文件 |
...
导出包含目标会话目录内的所有文件。全局诊断日志（`~/.echadron/logs/kimi-code.log`）默认包含，因为它可能含有其他会话或项目的事件；不想分享时加 `--no-include-global-log`。
...
立即检查最新版本并展示更新提示，选择操作后退出。也可以使用别名 `echadron update`。

```sh
echadron upgrade
```

对全局 npm、pnpm、yarn、bun 以及 macOS / Linux native 安装，`echadron upgrade` 会展示更新选项；选择 `Install update now` 后运行对应的前台安装命令。当前安装方式无法自动升级时（如 Windows native 安装），改为打印手动更新命令。
...
```sh
echadron provider catalog list anthropic          # 先看可选的模型
