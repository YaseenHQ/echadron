...
启动后命令不会打印任何 banner，立刻等待 ACP 客户端在 stdin 上发出 `initialize` 请求。日志会写到标准错误（以及 `~/.echadron/logs/` 下的诊断日志），所以 ACP 通道本身保持干净。
...
| 方法 | 状态 | 说明 |
| --- | --- | --- |
| `initialize` | 是 | 版本协商；返回 `agentInfo: { name: 'Echadron', version }`、能力矩阵、`authMethods` |
