...
| `ECHADRON_TIPS_BANNER_URL` | Opt in to a remote TUI banner feed. Echadron does not load provider promotional banners by default | An `http://` or `https://` JSON feed URL; blank values are ignored |
...
| `KIMI_GOAL_GATE_COMMANDS` | Commands that must exit 0 before a `/goal` complete request is accepted; takes higher priority than `[goal_completion_gates] commands` in `config.toml` | Semicolon- or newline-separated commands, or a JSON string array such as `["npm test"]`; blank or invalid values are ignored |
| `KIMI_GOAL_GATE_TIMEOUT_MS` | Per-command timeout (ms) for goal completion gates; takes higher priority than `[goal_completion_gates] timeout_ms` in `config.toml` (default `300000`) | Positive integer; invalid values fall back to the config or default |
| `ECHADRON_EXPERIMENTAL_TOOL_SELECT` | Override the default-on progressive MCP tool-disclosure feature; use a falsy value as a rollback control | Truthy: `1`/`true`/`yes`/`on`; falsy: `0`/`false`/`no`/`off` |
| `KIMI_CODE_EXPERIMENTAL_TOOL_SELECT` | Historical alias for `ECHADRON_EXPERIMENTAL_TOOL_SELECT` | Truthy or falsy values as above |
| `ECHADRON_EXPERIMENTAL_SECONDARY_MODEL` | Override the default-on secondary-model feature; use a falsy value as a rollback control | Truthy: `1`/`true`/`yes`/`on`; falsy: `0`/`false`/`no`/`off` |
| `KIMI_CODE_EXPERIMENTAL_SECONDARY_MODEL` | Historical alias for `ECHADRON_EXPERIMENTAL_SECONDARY_MODEL` | Truthy or falsy values as above |
| `ECHADRON_EXPERIMENTAL_PERSISTENCE_MINIDB_READMODEL` | Override the default-on minidb-derived session read model; use a falsy value as a rollback control | Truthy: `1`/`true`/`yes`/`on`; falsy: `0`/`false`/`no`/`off` |
| `KIMI_CODE_EXPERIMENTAL_PERSISTENCE_MINIDB_READMODEL` | Historical alias for `ECHADRON_EXPERIMENTAL_PERSISTENCE_MINIDB_READMODEL` | Truthy or falsy values as above |
| `KIMI_SECONDARY_MODEL` | Secondary model; takes higher priority than `[secondary_model] model` in `config.toml`. Newly spawned subagents (`Agent` / `AgentSwarm`) bind to it by default instead of inheriting the main agent's model | A model id from your configured `[models]`, e.g. `echadron-code/kimi-k2.5`; blank values are ignored |
| `KIMI_SECONDARY_EFFORT` | Thinking effort for the secondary model; takes higher priority than `[secondary_model] default_effort` in `config.toml` and applies when a secondary model is configured | An effort value, e.g. `low`; blank values are ignored |
...
| `ECHADRON_EXPERIMENTAL_FLAG` | Enable all registered feature controls for this process; retained for compatibility and diagnostics | `1`, `true`, `yes`, `on` |
| `KIMI_CODE_EXPERIMENTAL_FLAG` | Historical alias for `ECHADRON_EXPERIMENTAL_FLAG` | `1`, `true`, `yes`, `on` |
