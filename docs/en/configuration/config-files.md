...
```sh
export ECHADRON_HOME=/path/to/echadron-home
```
...
This feature is experimental and disabled by default. Under `echadron web`, enable it with `KIMI_CODE_EXPERIMENTAL_SECONDARY_MODEL=1`. Under `echadron -p`, `KIMI_CODE_EXPERIMENTAL_FLAG=1` is already required to select the v2 engine and also enables this feature. The interactive TUI ignores the configuration.
...
::: tip
MCP server declarations are configured in `~/.echadron/mcp.json` or the project-local `.echadron/mcp.json`, not in `config.toml`. The legacy `.kimi-code/mcp.json` path is also read when present. The interactive configuration entry point is `/mcp-config`; see [Model Context Protocol](../customization/mcp.md).
:::
...
In addition to the user-level files under `~/.echadron`, Echadron reads a project-local configuration file at `<project-root>/.echadron/local.toml`. It holds settings that are specific to one project checkout and typically should not be shared with teammates. The legacy `<project-root>/.kimi-code/local.toml` path remains supported.
