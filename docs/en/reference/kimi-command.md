...
`--agent` and `--agent-file` select which agent drives a new session, in both print mode (`echadron -p`) and the interactive TUI:

```sh
echadron --agent reviewer
echadron -p --agent reviewer "Review the changes on this branch"
```
...
Multiple instances can share one home directory: each registers itself under `~/.echadron/server/instances/`, and a busy port is retried with `port + 1` (58628, 58629, …).
...
Validate `config.toml` and `tui.toml` without starting the TUI or modifying either file. By default, the command checks the files under `ECHADRON_HOME` (or `~/.echadron` when the environment variable is unset). Missing default files are reported as skipped because built-in defaults can apply.
...
The export contains all files in the target session directory. The global diagnostic log (`~/.echadron/logs/kimi-code.log`) is included by default because it may contain events from other sessions or projects; add `--no-include-global-log` if you do not want to share it.
