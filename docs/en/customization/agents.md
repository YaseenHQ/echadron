...
Echadron includes three built-in sub-agents, ready to use out of the box, each aimed at a different task shape:
...
Echadron discovers agent files by scope; more specific scopes take higher priority: **Explicit (`--agent-file`) > Project > Extra > User > Built-in**. When two files define the same `name`, the higher-priority scope wins. Each directory is scanned recursively for `.md` files.

**User level** (applies to all projects):
- `$ECHADRON_HOME/agents/` (default: `~/.echadron/agents/`)
- `~/.agents/agents/`

The Kimi-specific user agent directory moves with `ECHADRON_HOME`, while the generic `~/.agents/agents/` directory stays under the real OS home so it can be shared across tools.
...
Two CLI flags select which agent drives a new session, in both print mode (`echadron -p`) and the interactive TUI:
