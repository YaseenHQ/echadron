...
> Hook configuration lives under the canonical Echadron data root (`ECHADRON_HOME`, default `~/.echadron`). The legacy `IMPERIUM_HOME` / `KIMI_CODE_HOME` variables remain supported as aliases.
...
```toml
# Written in ~/.echadron/config.toml
[[hooks]]
...
All hook rules are written in the `[[hooks]]` array in `~/.echadron/config.toml`, where each entry is one rule:
...
event = "PreToolUse"
matcher = "Bash"
command = "node ~/.echadron/hooks/block-dangerous-bash.mjs"
timeout = 5
```
...
After blocking, Echadron CLI writes the blocking reason back into the context, and the model can use this to choose a safer alternative.
