...
::: info
Echadron can import data from the legacy Python/uv `kimi-cli`. The migration copies compatible configuration, MCP declarations, and session history into Echadron's separate data directory without replacing the old installation.
:::
...
The **first time you run `echadron`**, it automatically checks whether kimi-cli data exists under `~/.kimi/`. If it finds any, a migration prompt appears, and you can choose to migrate now, do it later, or never be asked again.
