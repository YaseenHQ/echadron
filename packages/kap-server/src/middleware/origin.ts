 ...
 *     explicit whitelist (`ECHADRON_CORS_ORIGINS`, no `*` wildcard — PLAN
...
/**
 * Parse `ECHADRON_CORS_ORIGINS` into an allowlist.
 *
...
export function parseCorsOrigins(env: NodeJS.ProcessEnv = process.env): string[] {
  const raw = env['ECHADRON_CORS_ORIGINS'] ?? env['KIMI_CODE_CORS_ORIGINS'];
  if (raw === undefined) {
    return [];
  }
  return raw
