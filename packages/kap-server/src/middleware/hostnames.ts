...
export interface HostCheckOptions {
  ...
  /** Extra allowed hosts / domain-suffix patterns (from `ECHADRON_ALLOWED_HOSTS`). */
  readonly extra?: readonly string[];
  /** Disable the check entirely (`ECHADRON_DISABLE_HOST_CHECK=1`; test-only). */
...
/**
 * Parse `ECHADRON_ALLOWED_HOSTS` into an `extra` allowlist.
 *
...
export function parseAllowedHosts(env: NodeJS.ProcessEnv = process.env): string[] {
  const raw = env['ECHADRON_ALLOWED_HOSTS'] ?? env['KIMI_CODE_ALLOWED_HOSTS'];
  if (raw === undefined) {
    return [];
  }
  return raw
...
/** True when `ECHADRON_DISABLE_HOST_CHECK=1` (test/controlled env only). */
export function isHostCheckDisabled(env: NodeJS.ProcessEnv = process.env): boolean {
  return (env['ECHADRON_DISABLE_HOST_CHECK'] ?? env['KIMI_CODE_DISABLE_HOST_CHECK']) === '1';
}
...
export function formatHostErrorMessage(host: string | undefined): string {
  ...
  return `Invalid Host header: ${hostLabel}; allow this host with ECHADRON_ALLOWED_HOSTS=${hostArg} or 'echadron web --allowed-host ${hostArg}'.`;
}
