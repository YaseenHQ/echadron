...
The Echadron runtime resolves the data directory as `ECHADRON_HOME` first, falling back to `~/.echadron`. Legacy `KIMI_CODE_HOME` and `IMPERIUM_HOME` values are still accepted. Theme files live inside the `themes/` subdirectory of that data directory.
...
```
https://yaseenhq.github.io/echadron/en/customization/themes.html
```
