...
describe('telemetry bootstrap', () => {
  it('matches the Echadron and legacy Kimi disable-env true-value semantics', () => {
    expect(isTelemetryDisabledByEnv({ ECHADRON_DISABLE_TELEMETRY: '1' })).toBe(true);
    expect(isTelemetryDisabledByEnv({ ECHADRON_DISABLE_TELEMETRY: 'yes' })).toBe(true);
    ...
    expect(isTelemetryDisabledByEnv({ ECHADRON_DISABLE_TELEMETRY: '0' })).toBe(false);
    expect(isTelemetryDisabledByEnv({ ECHADRON_DISABLE_TELEMETRY: 'false' })).toBe(false);
  ...
  it('disables the singleton without attaching a sink when opted out', async () => {
    ...
    const savedEchadron = process.env['ECHADRON_DISABLE_TELEMETRY'];
    const savedKimi = process.env['KIMI_DISABLE_TELEMETRY'];
    try {
      process.env['ECHADRON_DISABLE_TELEMETRY'] = 'true';
      initializeTelemetry({
        homeDir: await tempHome(),
        deviceId: 'dev',
        appName: 'echadron-cli',
      ...
      if (savedEchadron === undefined) delete process.env['ECHADRON_DISABLE_TELEMETRY'];
      else process.env['ECHADRON_DISABLE_TELEMETRY'] = savedEchadron;
      if (savedKimi === undefined) delete process.env['KIMI_DISABLE_TELEMETRY'];
      else process.env['KIMI_DISABLE_TELEMETRY'] = savedKimi;
    }
